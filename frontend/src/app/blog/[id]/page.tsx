...
const BlogPage = () => {
  ...
  async function fetchComment() {
    try {
      setLoading(true);
      const { data } = await axios.get<Comment[]>(`${blog_service}/api/v1/comment/${id}`);
      setComments(data);
    } catch (error) {
      console.log(error);
    } finally {
  ...
  async function addComment() {
    try {
      ...
      const { data } = await axios.post<{message:string}>(`${blog_service}/api/v1/comment/${id}`,
        { comment },
        {
          headers: {
  ...
  async function fetchSingleBlog() {
    try {
      setLoading(true);
      const { data } = await axios.get<{blog:Blog,author:User}>(`${blog_service}/api/v1/blog/${id}`);
      setBlog(data.blog);
  ...
  const deleteComment = async (id: string) => {
    if (confirm("Are you sure you want to delete this comment")) {
      try {
        setLoading(true);
        const token = Cookies.get("token");
        const { data } = await axios.delete<{message:string}>(`${blog_service}/api/v1/comment/${id}`,
          {
            headers: {
              Authorization: `Bearer ${token}`,
            },
  ...
  async function saveBlog() {
    ...
    try {
      setLoading(true);
      const { data } = await axios.post<{ message: string }>(`${blog_service}/api/v1/save/${id}`,
        {},
        {
          headers: {
            Authorization: `Bearer ${token}`,
