...
const ProfilePage = () => {
  ...
  const changeHandler = async (e: any) => {
    ...
    if (file) {
      ...
      try {
        ...
        const { data } = await axios.post<{ message: string; token: string; user: any }>(`${user_service}/api/v1/user/update/pic`,
          formData,
          {
            headers: {
  ...
  const handleFormSubmit = async () => {
    try {
      setLoading(true);
      const token = Cookies.get("token");
      const { data } = await axios.post<{ message: string; token: string; user: any }>(
